...
export class Snake extends AcGameObject{
    constructor(info,gamemap){
        ...
        this.speed= 5;
        this.direction= -1;//表示没有指令，0、1、2、3、4表示上下左右
        ...
        if(this.id===1) this.eye_direction=2;//左边蛇眼睛向上，右边向下
    ...
    check_tail_increasing(){//判断当前回合蛇的长度是否增加
    ...
    render(){
        ...
        ctx.fillStyle="black";
        for(let i=0;i<2;i++){
            const eye_x=(this.cells[0].x+this.eye_dx[this.eye_direction][i]*0.35)*L;
            const eye_y=(this.cells[0].y+this.eye_dy[this.eye_direction][i]*0.35)*L;
